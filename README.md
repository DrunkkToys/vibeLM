...
| `tools.enforceMainChatBounds` | boolean | `true` | vibeLM owns the main chat's prediction loop (via `PredictionLoopHandler`) so `Max Orchestrator Turns` and the always-reasoning token floor actually cap the interactive chat, not just `vibe_bridge` ticks. Turn off to remove those caps if they cause problems — vibeLM still renders the chat either way, since LM Studio has no way to hand the loop back to its own default handler once this is registered. |
...
- `maxOrchestratorTurns` defaults to `50`, accepts values from `0` to `100`, and `0` disables the hard turn cap. It's enforced twice: on every tool call (via the tools provider) and, since `Enforce Main Chat Bounds` takes over the main chat's prediction loop, as a real `maxPredictionRounds` cap on the interactive chat itself — so a model that never calls another tool after a failure can no longer ramble unbounded (previously only `vibe_bridge` ticks had this protection; the main channel had none).
