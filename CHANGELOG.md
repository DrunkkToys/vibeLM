...
### Fixed
- Tool failures now return actionable normal output to LM Studio, allowing the model to explain the
  problem or choose another approach instead of ending the turn.
- Web search now uses DuckDuckGo first and falls back to Bing RSS, restoring relevant technical
  research results when Bing returns unrelated matches.
